...
DOCKER_IMAGE=sadubois/employeedb:1.4.2
CONTAINER_PORT=8080
EXPOSE_PORT=8080
...
ret=1; while [ $ret -ne 0 ]; do
  nc -w1 -z $DB_SERVER $DB_PORT; ret=$?
...
docker run -d \
  ...
  -e SPRING_DATASOURCE_URL="jdbc:mysql://$DB_SERVER:$DB_PORT/employeedb?createDatabaseIfNotExist=true" \
