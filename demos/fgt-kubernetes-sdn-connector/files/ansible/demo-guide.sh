...
cat $HOME/workspace/tdh-vsphere-tkgs-sdubois-svc-kubeconfig > $HOME/.kube/config
  ...
  if [ ! -d $HOME/PythonDev ]; then 
    ...
    pip3 install -r $HOME/workspace/tanzu-demo-hub/demos/$TDH_DEMO_NAME/scripts/requirements.txt | sed 's/^/   /'
    echo "   ---------------------------------------------------------------------------------------------------------------------------------------------------------------"
  fi
