#!/bin/bash
#===============================================================================
# SCRIPT NAME:    debcli_MBP14_deCH/addons.sh
...
mkdir -p $BUILDDIR/fortipoc && cat $FABRIC_HOME/modules/debcli_postinst | sed \
  ...
  -e "s/KUBECTL_CLIENT=0/KUBECTL_CLIENT=$KUBECTL_CLIENT/g"                                                      >  $BUILDDIR/fortipoc/postinst
...
[ -f $DEMOPATH/files/etc/motd_cli ] && cat $DEMOPATH/files/etc/motd_cli >> $BUILDDIR/etc/motd
